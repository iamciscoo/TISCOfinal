/**
 * PDF Receipt Generator for TISCO Orders
 * Generates professional order receipts using jsPDF
 */
...
// TZS to USD conversion rate (approximate)
const TZS_TO_USD = 0.00040 // 1 TZS ≈ 0.0004 USD (2500 TZS = 1 USD)

/**
 * Format currency
 */
function formatCurrency(amount: number, currency: 'TZS' | 'USD'): string {
  if (currency === 'USD') {
...
export async function generateOrderReceipt(order: Order): Promise<jsPDF> {
  ...
  doc.setFont('helvetica', 'normal')
  doc.text(order.payment_status.toUpperCase(), rightCol + 32, yPos)
  ...
  const tableData = items.map((item, index) => {
    ...
    const productName = truncate(product?.name || `Product ${item.product_id.slice(0, 8)}`)
...
export async function downloadReceipt(order: Order): Promise<void> {
  ...
  const fileName = `TISCO_Receipt_${order.id.slice(0, 8)}_${Date.now()}.pdf`
  doc.save(fileName)
}

/**
 * Generate receipt as blob for upload or email
 */
export async function generateReceiptBlob(order: Order): Promise<Blob> {
  const doc = await generateOrderReceipt(order)
