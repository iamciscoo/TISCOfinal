...
export default function PublicEmailStudioPage() {
      ...
      <div className="max-w-7xl mx-auto p-6 grid grid-cols-1 lg:grid-cols-4 gap-6">
        ...
        <div className="lg:col-span-1">
          <div className="bg-white rounded-xl shadow-lg p-6 sticky top-6">
            <h3 className="text-lg font-semibold text-slate-800 mb-4">Email Events ({Object.keys(emailEvents).length})</h3>
            ...
            {error && Object.keys(emailEvents).length === 0 && (
            ...
            <div className="flex items-center justify-between flex-wrap gap-4">
              <div className="flex items-center space-x-3">
                <div className={`w-3 h-3 rounded-full ${
                  loading ? 'bg-yellow-500 animate-pulse' : error ? 'bg-red-500' : currentTemplate ? 'bg-green-500' : 'bg-gray-400'
                }`}></div>
              ...
              <div className="flex items-center space-x-2">
                ...
                <button
                  ...
                  className="px-3 py-2 bg-green-100 text-green-700 hover:bg-green-200 rounded text-sm font-medium disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
                >
                  💾 Export
                </button>
              </div>
          ...
          <div className="bg-white rounded-xl shadow-lg overflow-hidden">
            <div className="bg-slate-50 px-4 py-2 border-b">
              <div className="flex items-center justify-between text-sm">
                ...
                <span className="text-slate-500">
                  {currentViewMode === 'mobile' ? '375×812px' : 'Responsive'}
                </span>
              </div>
            </div>
            ...
            <div className="h-[600px] relative bg-slate-50">
              {loading && (
                <div className="absolute inset-0 bg-white bg-opacity-90 flex items-center justify-center z-10">
                  <div className="flex items-center">
                    ...
                    <span className="ml-3 text-slate-600 font-medium">Loading template...</span>
                  </div>
                </div>
              )}
              ...
              {error && !loading && (
                <div className="absolute inset-0 flex items-center justify-center">
                  <div className="text-center max-w-md">
                    ...
                    <p className="text-red-600 font-medium text-lg mb-2">Error loading template</p>
                    <p className="text-red-500 text-sm bg-red-50 p-3 rounded">{error}</p>
                    <button
                      onClick={() => currentEventType && selectEmailEvent(currentEventType)}
              ...
              {!currentTemplate && !loading && !error && (
                <div className="absolute inset-0 flex items-center justify-center">
                  <div className="text-center text-slate-500">
                    ...
                    <p className="text-xl font-medium mb-2">Welcome to Email Template Studio</p>
                    <p className="text-sm">Select an email template from the sidebar to preview</p>
                    <p className="text-xs mt-2 text-slate-400">
                      {Object.keys(emailEvents).length} templates available
                    </p>
                  </div>
                </div>
              )}
              ...
              {currentTemplate && !loading && (
                <div className={`h-full ${currentViewMode === 'mobile' ? 'flex items-center justify-center p-4' : ''}`}>
                  <div className={
                    currentViewMode === 'mobile' 
                      ? 'w-[375px] h-[600px] border-8 border-slate-800 rounded-[20px] overflow-hidden shadow-2xl bg-white' 
                      : 'w-full h-full'
                  }>
                    <iframe
